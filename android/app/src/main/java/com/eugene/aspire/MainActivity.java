...
public class MainActivity extends FlutterActivity {
    ...
    private boolean userDeniedOnce = false;
...
// ✅ ADD MAP (For Flutter Timer & API)
                    ...
                    map.put("walkSteps", wSteps);
                    map.put("runSteps", rSteps);
                    map.put("sleepSeconds", sleepSecond);
                    map.put("sleepState", sleepState);
                    map.put("heartRate", heartRate);
                    map.put("is_wear", isWear);
                    map.put("fspo2", spo2);
                    map.put("hrv", hrv);
                    map.put("bp_high", bphp);
                    map.put("bp_low", bplp);
...
// ✅ Send MAP (for logic)
                ...
                public void onGetDayTotalData(DayTotalDataModel data) {
                    ...
                    map.put("totalSleep", sleep);
                    map.put("totalSteps", step);
                    map.put("heartRate", hr);
                    map.put("iBPLow", bplp);
                    map.put("iBPHigh", bphp);
                    map.put("spo2", mnSpo2);
                    map.put("hrv", mdHrv);
    ...
    private void testRawBle() {
        Log.e("RAW_BLE", "RAW SCAN STARTED");
        BluetoothAdapter adapter =
                BluetoothAdapter.getDefaultAdapter();

        if (adapter == null) {
            Log.e("RAW_BLE", "BluetoothAdapter is NULL");
            return;
        }
        ...
        if (scanner == null) {
            Log.e("RAW_BLE", "BluetoothLeScanner is NULL");
            return;
        }
...
//ClingSdk.start(getApplicationContext());
    ...
    public void onRequestPermissionsResult(int requestCode,
        ...
        if (requestCode == 2001) {
            ...
            if (granted) {
                userDeniedOnce = false;
                Log.e(
                        "VIVO_DEBUG",
                        "FLUTTER REQUESTED STARTSCAN"
                );
                testRawBle();
                new Handler(Looper.getMainLooper()).postDelayed(() -> {
                ...
                if (!shouldShowRationale) {
                    // System forcibly denied without popup (e.g. user selected "Don't ask again")
                    openAppSettings();
                } else {
                    userDeniedOnce = true;
                }
                    ...
                    case "startScan":
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                            ...
                            if (!isGpsEnabled && !isNetworkEnabled) {
                                result.error("LOCATION_DISABLED", "Location services must be enabled", null);
                                return;
                            }
                        }
                        Log.e(
...
// ClingBleManager.connect(call.argument("name"), this);
                    ...
                    case "clearNativeCache":
                        Log.i(TAG, "🧹 Local Logout: Clearing native device maps and database to simulate uninstall amnesia");
                        getSharedPreferences("CLING_PREF", MODE_PRIVATE)
                                .edit()
                                .clear()
                                .apply();
                        ClingBleManager.clearNativeCache();
                        result.success(null);
                        break;
                    case "stopScan":
...
// Try via reflection (works for most SDK builds)
                    ...
                    case "autoReconnect": {
                        ...
                        Log.i(TAG, "🔄 Starting native 2-stage reconnect for clingId: " + clingId);
                        ClingSdk.stopScan();
                        ClingSdk.setUserId(USER_ID);

                        // Stage 1: Discover
                        // We must let the scan run so the native SDK caches the Cling device internally
                        ClingSdk.startScan(15000, new OnBleListener.OnScanDeviceListener() {
                            @Override
                        ...
                        // Wait a robust 4 seconds for the hardware to broadcast its ping, then connect!
                        new Handler(Looper.getMainLooper()).postDelayed(() -> {
                            Log.i(TAG, "✅ 2nd Stage: 4-sec scan complete. Connecting to clingId=" + clingId);
                            ClingSdk.stopScan();
                            ClingSdk.connectDeviceByClingid(USER_ID, clingId);
                        }, 4000);
                        result.success(null);
