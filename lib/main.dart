...
//       debugShowCheckedModeBanner: false,
//       home: kIsWeb ? LoginScreen() : SplashPage(),
//     );
//   }
// }
//
...
class _SplashPageState extends State<SplashPage> {
  ...
  Future<void> checkSession() async {
    ...
    if (!loggedIn) {
      // Not logged in → navigate to login after 3 sec
      Future.delayed(const Duration(seconds: 3), () {
        if (mounted) Get.offAll(() => LoginScreen());
      });
      return;
    }

    // If logged in → check last active device
    try {
      final String? lastDeviceID = await platform.invokeMethod('getLastActiveDevice');
      ...
      if (lastDeviceID != null && lastDeviceID.isNotEmpty) {
        ...
        // Navigate to dashboard after 3 sec
        Future.delayed(const Duration(seconds: 3), () {
          if (mounted && _registeredDevice != null) {
            Get.offAll(() => DashboardScreen(deviceID: _registeredDevice!));
          }
        });
        // } else {
      ...
      print("Failed to get last active device: ${e.message}");
      // If an error occurs → go to BluetoothPair after 3 sec
    }
  }
