...
class _SplashPageState extends State<SplashPage> {
  ...
  bool isLoading = true;
  static const platform = MethodChannel('cling_sdk');
  static const _method = MethodChannel("cling/methods");
  ...
  Future<void> checkSession() async {
    ...
    // // ✅ ANDROID: always listen first
    // if (Platform.isAndroid) {
    //
    //   _method.setMethodCallHandler((call) async {
    //     if (call.method == "deviceReady") {
    //
    //       final clingId = call.arguments;
    //
    //       if (clingId != null) {
    //         Get.offAll(() => DashboardScreen(deviceID: clingId));
    //       } else {
   ...
   if(Platform.isIOS){
    ...
    if(Platform.isAndroid){
      if(_lastdeviceId != null){
        Get.to(() => DashboardScreen(deviceID: _lastdeviceId!));
      }
    }
  }
    ...
    return EasySplashScreen(
     ...
     //      : null,
      navigator: isLoggedIn
          ...
          : BluetoothPair()))
          : LoginScreen(),
     //  navigator: isLoggedIn ? null : LoginScreen(),
    );
  }
}
