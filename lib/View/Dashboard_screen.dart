...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  final ToggleStatusController _toggleController = ToggleStatusController();
  String t1 = "";
  String t2 = "";
  String datetime = "";
  String heartRate = "";
 ...
 // bool shouldSync = false;
  ...
  Future<void> _loadToggleStatuses() async {
    ...
    for (var t in toggles) {
      // 🧠 Use saved local value if API cannot fetch directly
      final prefs = await SharedPreferences.getInstance();
      final saved = prefs.getBool(t["name"]) ?? false;
      (t["key"] as ValueNotifier<bool>).value = saved;

      print("📦 Loaded ${t["name"]} locally → ${saved ? "ON" : "OFF"}");
    }
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          ...
                          // ),
                          ValueListenableBuilder<bool>(
                            valueListenable: settings.showWeight,
                            builder: (context, show, _) {
                              return show
                                  ? CustomCard(
                                title: AppText.weight,
