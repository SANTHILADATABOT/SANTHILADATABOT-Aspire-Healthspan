...
import 'dart:io';
import 'dart:ui';
import 'package:aspire/Controller/editprofile_controller.dart';
import 'package:aspire/View/edit_emai.dart';
...
class _EditProfileState extends State<EditProfile> {
  ...
  Future<void> show_dialogue() async {
    ...
    // No need to check READ_MEDIA_IMAGES or Permission.photos anymore
    showDialog(
      context: context,
  ...
  Future<void> _handleImageSelectionGallery() async {
    final XFile? result = await _picker.pickImage(
      source: ImageSource.gallery, // Android Photo Picker (no permission needed)
      imageQuality: 70,
      maxWidth: 1440,
    );
    ...
    if (result != null) {
      setState(() {
        shopImage = File(result.path);
      });
    }
  }

  Future<void> _handleImageSelectionCamera() async {
    final XFile? result = await _picker.pickImage(
                ...
                child: Stack(
                  children: [
                    ...
                    Positioned(
                      top: 90,
                      left: 85,
                      child: Icon(Icons.linked_camera_sharp, size: 20,
