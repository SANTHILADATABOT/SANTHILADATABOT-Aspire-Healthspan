...
class SleepController extends GetxController{
  ...
  }) async
  {
    ...
    print("Day_sleep_chart_Api:${response.body}");
  ...
  Future<Map<String, dynamic>> Week_Chart({
    ...
    print("week_sleep_chart_Api:${response.body}");
    ...
    (jsonResponse['sleep_data'] ?? {}) as Map<String, dynamic>;
    ...
    /// 🔥 MAIN LOOP – ALL 7 DAYS GUARANTEED
    for (final dayKey in orderedDays) {
      ...
      final deepRaw = summary['3'] ?? '0h 0m';
      final remRaw = summary['4'] ?? '0h 0m';
      final totalRaw = summary['total'] ?? '0h 0m';

      final lightSleepHours = parseDurationToHours(lightRaw);
      final deepSleepHours = parseDurationToHours(deepRaw);
      final remSleepHours = parseDurationToHours(remRaw);
      final totalSleepHours = parseDurationToHours(totalRaw);

      if (totalSleepHours > maxTotalSleep) {
        maxTotalSleep = totalSleepHours;
      }

      final label = daysOfWeekMap[dayKey]!;
      ...
      if (lightSleepHours > 0) {
        ...
        weeklyBars.add(SleepBarData(
          ...
          endHour: currentStart + lightSleepHours,
          value: 2,
        ));
        currentStart += lightSleepHours;
      }
      ...
      if (remSleepHours > 0) {
        ...
        weeklyBars.add(SleepBarData(
          label: label,
          startHour: currentStart,
          endHour: currentStart + remSleepHours,
          value: 4,
        ));
        currentStart += remSleepHours;
      }
      ...
      if (deepSleepHours > 0) {
        ...
        weeklyBars.add(SleepBarData(
          label: label,
          startHour: currentStart,
          endHour: currentStart + deepSleepHours,
          value: 3,
        ));
      }

      /// ✅ FORCE X-AXIS LABEL WHEN SLEEP = 0
      if (!hasData) {
        weeklyBars.add(SleepBarData(
          label: label,
          startHour: 0,
  ...
  }) async
  {
    ...
    print("month_sleep_chart_Api:${response.body}");
    if (response.statusCode == 200) {
      ...
      for (int i = 1; i <= totalDays; i++) {
        ...
        // Determine which total to show (calculated vs API total)
        String finalTotalSleep;
        if (totalSleepRaw == '0h 0m' && total > 0) {
          finalTotalSleep = formatHourMinute(total);
        } else {
          finalTotalSleep = totalSleepRaw;
        }

        // ALWAYS add bars, even if zero (for visual continuity)
        // But only add if there's data OR if you want to show zeros
        double lightStart = 0;
        double lightEnd = lightStart + light;
        double remStart = lightEnd;
        double remEnd = remStart + rem;
        ...
        // Always add a bar with value 5 (or any placeholder) for zero days
        if (total == 0) {
          // Add a placeholder bar with zero height (invisible)
          monthlyBars.add(SleepBarData(
            label: i.toString(),
            startHour: 0,
            endHour: 0,
            value: 5, // Use a value that represents "no data" or is transparent
          ));
        } else {
          // Add bars only when there's actual sleep
          if (light > 0) {
            monthlyBars.add(SleepBarData(
              label: i.toString(),
              startHour: lightStart,
  ...
  }) async
  {
    ...
    print("yearly_sleep_chart_Api:${response.body}");
...
//   }
  ...
  Future<Map<String, dynamic>> MultiYear_Chart({
    ...
    print("multiyear_sleep_chart_Api:${response.body}");
